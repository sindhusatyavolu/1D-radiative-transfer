...
double evolve_eqns(double *nh,double **nh2,double **NH,double **nhe1,double **nhe2,double **nhe3,double **NHe1,double **NHe2,double **T,int ncells,double ti,double tsim,double **dt, int j,double Dt){
	...
	out = xTiter(dt[k][j],NH[k][j],NHe1[k][j],NHe2[k][j],(5+k)*cellsize,nh[k],tsim,ti,Tt,xh2t,xhe1t,xhe2t,xhe3t,&dt_rec);  // equivalent of cool_step routine
	//printf("%d\n",out);
	...
	out = xTiter(dt[k][j],NH[k][j],NHe1[k][j],NHe2[k][j],(5+k)*cellsize,nh[k],tsim,ti,Tt,xh2t,xhe1t,xhe2t,xhe3t,&dt_rec);
...
//	printf("dt new out %e \n",dt[0][j+1]);
   ...
   if (cks==ncells-1)
   {
        Dt = 1e-1;//3261.6*(epsilon/0.1)*(cellsize/10.0)*1e-6;//nm^-2//*(pow(3e21,2))/1e-14 ; // in kpc^-2 
   }
   else{
...
FILE *fptr = fopen("t2dxh1_1myr.txt","w");
FILE *fdptr = fopen("t2dxhe2_1myr.txt","w");
FILE *fd1ptr = fopen("t2dxhe3_1myr.txt","w");
FILE *fd2ptr = fopen("t2dT_1myr.txt","w");
FILE *fd3ptr = fopen("t2dxhe1_1myr.txt","w");
/*
